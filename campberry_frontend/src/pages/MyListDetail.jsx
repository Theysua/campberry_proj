...
export default function MyListDetail() {
  ...
  if (loading) return <div className="page" style={{ padding: '40px', textAlign: 'center' }}>Loading...</div>;
  if (!list) return <div className="page" style={{ padding: '40px', textAlign: 'center' }}>List not found.</div>;
  ...
  const handleMove = async (index, direction) => {
    ...
    if (direction === 1 && index === list.items.length - 1) return;
            ...
            {isEditingList && isOwner ? (
              <div className="flex flex-col gap-4 mb-6">
                <div>
                  ...
                  <input className="w-full border border-slate-200 rounded-lg p-2.5 text-sm outline-none focus:border-[#892233]" value={editTitle} onChange={e => setEditTitle(e.target.value)} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#011936] mb-1">Description</label>
                  <textarea className="w-full border border-slate-200 rounded-lg p-2.5 text-sm outline-none focus:border-[#892233] h-24 resize-none" value={editDesc} onChange={e => setEditDesc(e.target.value)} />
                </div>
            ...
            <div className="flex flex-col gap-4">
              {list.items && list.items.length > 0 ? list.items.map((item, index) => (
                  ...
                  <div className="w-full md:w-5/12 bg-slate-50 rounded-xl p-4 border border-slate-100 relative mt-4 md:mt-0 md:min-h-[100px] flex flex-col justify-center">
                    ...
                    {editingItemId === item.id ? (
                      <div className="flex flex-col gap-2 relative z-20">
                        <textarea className="w-full text-sm p-2 border border-[#892233] focus:ring-1 focus:ring-[#892233] outline-none bg-white rounded-lg h-24 resize-none font-medium text-[#011936]" autoFocus placeholder="Add your reason why this program is a good fit..." value={editCommentary} onChange={e => setEditCommentary(e.target.value)} />
                ...
                <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-16 flex flex-col items-center justify-center text-center mt-2">
                  <div className="text-slate-400 mb-4 font-bold text-lg">No programs tracking in this list</div>
