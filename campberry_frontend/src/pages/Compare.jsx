...
export default function Compare() {
    ...
    useEffect(() => {
        ...
        const fetchDetails = async () => {
            setLoading(true);
            try {
                const promises = compareList.map(p => getProgramById(p.id));
                const results = await Promise.all(promises);
                setDetailedPrograms(results.filter(Boolean));
            } catch (err) {
    ...
    if (loading) return <div className="page" style={{ padding: '40px', textAlign: 'center' }}>Loading Comparison...</div>;
        ...
        <div className="page" id="page-compare">
            <div className="container" style={{ maxWidth: '1400px', minHeight: '60vh' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                    <div>
                        <button className="btn-outline" onClick={() => navigate('/search')} style={{ marginBottom: '16px', fontSize: '13px', padding: '6px 18px' }}>← Back to Search</button>
                        ...
                        <div key={program.id} className="card" style={{ flex: '1', minWidth: '300px', maxWidth: '400px', display: 'flex', flexDirection: 'column' }}>
                            <div style={{ padding: '24px', borderBottom: '1px solid var(--border-light)', position: 'relative' }}>
                                ...
                                <div style={{ width: '64px', height: '64px', background: program.logo_url ? '#ffffff' : 'linear-gradient(135deg, #f1f5f9, #e2e8f0)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', marginBottom: '16px' }}>
                                    {program.logo_url ? <img src={program.logo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '4px' }} /> : (program.type === 'COMPETITION' ? '🏆' : '🎓')}
                                </div>
                            ...
                            <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                        ...
                                        {program.allows_international && <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        {program.sessions?.slice(0, 2).map((s, i) => (
                                            <div key={i} style={{ background: 'var(--bg-alt)', padding: '8px 12px', borderRadius: 'var(--radius-sm)' }}>
                                                <div style={{ fontWeight: '600', marginBottom: '2px' }}>{s.location_type} {s.location_name ? `- ${s.location_name}` : ''}</div>
                                                {s.start_date && <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{new Date(s.start_date).toLocaleDateString()} - {new Date(s.end_date).toLocaleDateString()}</div>}
                                            </div>
                                        ))}
